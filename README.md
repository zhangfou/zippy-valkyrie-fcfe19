...
### 3. 阶跃星辰 Step Plan / StepFunMCP 兼容

本项目已内置“阶跃星辰 Step Plan”API 提供商预设，可兼容 StepFun 推理模型文档中的 OpenAI 协议通道，也可沿用 [weidafeng/StepFunMCP](https://github.com/weidafeng/StepFunMCP) 的鉴权方式。

1. 打开**设置** → **API 连接与服务**。
2. 在 **API 提供商**中选择“阶跃星辰 Step Plan”。
3. 填入您的 StepFun API Key。
4. 默认接口地址为 `https://api.stepfun.com/step_plan/v1`，默认聊天模型为 `step-3.7-flash`。
5. 内置兼容模型包括 `step-3.7-flash`、`step-3.5-flash-2603`、`step-3.5-flash` 和 `step-router-v1`。点击“获取模型”可从 StepFun `/step_plan/v1/models` 拉取模型；如果拉取失败，应用会保留内置兼容模型作为兜底。
